   ...
   if(metric==0){
	...
	.on('contextmenu', function(event) {
  	...
  	console.log(this.apiURL+'/projects/'+file+"/"+file+"-CV.json")
		...
		.subscribe(resultado => {this.readPackageView(resultado as any[],d3.select(".svg-container-cv").attr("lastSelected"),metric,d3.select(".svg-container-cv").attr("lastSelected"))});
